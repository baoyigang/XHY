...
public class WareHouseTree : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string json = "[{\"id\":\"0\",\"text\":\"仓库de资料\",\"state\":\"open\"";
    ...
    private string GetWareHoseTree(string WareHouseCode)
    {
        ...
        for (int j = 0; j < dtArea.Rows.Count; j++)
        {
            ...
            string shelfTree = GetShelfTree(dr["AreaCode"].ToString());
    ...
    private string GetShelfTree(string AreaCode)
    {
        string json = "";
        BLL.BLLBase bll = new BLL.BLLBase();
        DataTable dtShelf = bll.FillDataTable("Cmd.SelectCell", new DataParameter[] { new DataParameter("{0}", string.Format(" AreaCode='{0}'", AreaCode)) });
        for (int j = 0; j < dtShelf.Rows.Count; j++)
        {
            DataRow dr = dtShelf.Rows[j];
